...
static void* createCommandPool(void* threadCommandPoolInfo) {
	...
	vulkRes = vkCreateCommandPool(*info->pDevice, &createInfo, VK_NULL_HANDLE, &commandpool);
	if (vulkRes) {
...
SgResult sgBeginCommandBuffer(const SgApp* pApp, SgCommandBufferBeginEndInfo *pBeginEndInfo) {
    ...
    vkAllocateCommandBuffers(pApp->device, &allocInfo, pBeginEndInfo->pCommandBuffer);
...
SgResult sgEndCommandBuffer(const SgApp* pApp, SgCommandBufferBeginEndInfo *pBeginEndInfo) {
	...
	vkWaitForFences(pApp->device, 1, pBeginEndInfo->pFence, true, 0xffffff);
...
SgResult sgTransferImage(const SgApp* pApp, SgImageTransferInfo* pTransferInfo) {
	...
	vkCmdPipelineBarrier(commandBuffer, pTransferInfo->srcStageMask, pTransferInfo->dstStageMask, pTransferInfo->dependencyFlags, 0, NULL, 0, NULL, 1, &barrier);
