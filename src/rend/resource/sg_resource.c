...
SgResult sgCreateResourceMap(const SgApp* pApp, SgResourceMap** ppResourceMap) {
	...
	pResourceMap->pResourceMap = hashmap_new(
	    sizeof(SgResource), 1, 0, 0, resourceKeyHash, resourceKeyCompare, NULL);
	...
	VkResult vulkRes =
	    vkCreateCommandPool(pApp->device, &commandPoolCreateInfo, VK_NULL_HANDLE,
	                        &pResourceMap->commandPool);
	if (vulkRes) {
...
static SgResult sgCreateResource(const SgApp* pApp,
	...
	if (pCreateInfo->type & SG_RESOURCE_TYPE_IS_IMAGE_MASK) {
		...
		if (SG_RESOURCE_TYPE_REQIRE_STAGING_MASK & pCreateInfo->type) {
			...
			vmaMapMemory(pApp->allocator, pResource->stagingBuffer.allocation, &data);
			pResource->stagingBuffer.bytes = data;
			memcpy(pResource->stagingBuffer.bytes, pCreateInfo->bytes,
			       pCreateInfo->size);
		} else {
			sgCreateBuffer(pApp, &bufferCreateInfo, &pResource->dataBuffer);
			vmaMapMemory(pApp->allocator, pResource->dataBuffer.allocation, &data);
			pResource->dataBuffer.bytes = data;
			memcpy(pResource->dataBuffer.bytes, pCreateInfo->bytes,
			       pCreateInfo->size);
		}
	}
...
_Bool sgInitResourcesIter(const void* item, void* data) {
	...
	*pResource->pCommandBufferID = iterInfo->iter;
	...
	if (pResource->type & SG_RESOURCE_TYPE_IS_IMAGE_MASK) {
		...
		vkCmdCopyBuffer(*pCommandBuffer, pResource->stagingBuffer.buffer,
		                pResource->dataBuffer.buffer, 1, &copyRegion);
	}
...
SgResult sgInitResourceMap(const SgApp* pApp, SgResourceMap** ppResourceMap) {
	...
	struct SgResourceIterInfo iterInfo = {
	    .pResourceMap = pResourceMap, .pApp = pApp, .iter = 0};
...
SgResult sgCreateBuffer(const SgApp* pApp,
	...
	VkResult res =
	    vmaCreateBuffer(pApp->allocator, &bufferCreateInfo, &allocationInfo,
	                    &pBuffer->buffer, &pBuffer->allocation, NULL);
	if (res) {
...
SgResult sgCreateImageView(const SgApp* pApp,
	...
	vkCreateImageView(pApp->device, &createinfo, VK_NULL_HANDLE,
...
SgResult sgUpdateResource(const SgApp* pApp,
	...
	SgResource* pResource =
	    hashmap_get(pResourceMap->pResourceMap, &findResource);
	if (pResource == NULL) {
	...
	if (pData->size > pResource->dataBuffer.size) {
		sgLogWarn(
		    "[Res]: Data size exceeds resource size. Possible memory corruption");
	}
...
SgResult sgUpdateAllResources(const SgApp* pApp, SgResourceMap* pResourceMap) {
	struct SgResourceIterInfo iterInfo = {
	    .pResourceMap = pResourceMap, .pApp = pApp, .iter = 0};
	hashmap_scan(pResourceMap->pResourceMap, sgImagesToTransferResourcesIter,
	             &iterInfo);
	...
	vkWaitForFences(pApp->device, 1, &pResourceMap->fence, VK_TRUE, UINT64_MAX);
	vkResetFences(pApp->device, 1, &pResourceMap->fence);
	hashmap_scan(pResourceMap->pResourceMap, sgImagesToRenderResourcesIter,
	             &iterInfo);
