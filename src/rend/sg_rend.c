...
static SgResult createGLFWwindow(const SgAppConfig* pAppConfig,
	...
	if (pAppConfig->windowConfig.createFlags & SG_APP_WINDOW_FULLSCREEN) {
		...
		pWindow = glfwCreateWindow(mode->width, mode->height,
		                           pAppConfig->windowConfig.pName, monitor, NULL);
	} else {
		pWindow = glfwCreateWindow(pAppConfig->windowConfig.dimentions[0],
		                           pAppConfig->windowConfig.dimentions[1],
		                           pAppConfig->windowConfig.pName, NULL, NULL);
	}
	if (pWindow) {
...
SgResult sgCreateApp(const SgAppCreateInfo* pCreateInfo, SgApp** ppApp) {
	...
	pApp->msaaSampleCount = (pApp->appConfig.graphicsConfig.mssa > maxSamples)
	                            ? maxSamples
	                            : pApp->appConfig.graphicsConfig.mssa;
	...
	getSupportedDepthStencilFormats(&pApp->physicalDevice, &pApp->lowDepthStencil,
	                                &pApp->highDepthStencil);

	getLogicalDevice(&logicalDeviceGetInfo, &pApp->device);
	vkGetDeviceQueue(pApp->device, pApp->graphicsQueueFamilyIdx, 0,
	                 &pApp->graphicsQueue);

	/* Create Vulkan Allocator */
	createAllocator(&pApp->physicalDevice, &pApp->device, &pApp->instance,
	                &pApp->allocator);

	/* Vulkan Surface Attirbutes */
	getSurfaceAttributes(&pApp->physicalDevice, &pApp->surface,
	                     &pApp->surfaceAttributes);

	/* Command Pool Initialization */
	pApp->commandPoolCount = SG_THREADS_COUNT;
	createCommandPools(&pApp->device, pApp->graphicsQueueFamilyIdx, pApp->threads,
	                   pApp->pCommandPools, pApp->commandPoolCount);
	...
	for (uint32_t i = 0; i < SG_FRAME_QUEUE_LENGTH; ++i) {
		...
		res |= vkCreateSemaphore(pApp->device, &semaphoreCreateInfo, VK_NULL_HANDLE,
		                         &pApp->pFrameFinishedSemaphore[i]);
		res |= vkCreateFence(pApp->device, &fenceCreateInfo, VK_NULL_HANDLE,
		                     &pApp->pFrameFences[i]);
	}
	if (res) {
...
_Bool renderPassBindRenderObjects(const void* item, void* udata) {
	...
	SgMaterial* pMaterial =
	    hashmap_get(pInfo->pMaterialMap->pMaterialMap, &materialGet);

	vkCmdBindPipeline(*pInfo->pCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
	                  pMaterial->pipeline);
...
SgResult sgInitUpdateCommands(const SgUpdateCommandsInitInfo* pInitInfo,
	...
	SG_STRETCHALLOC(pUpdateCommands->pCommandBuffers,
	                pInitInfo->pMaterialMap->swapchain.imageCount,
	...
	for (uint32_t i = 0; i < pInitInfo->pMaterialMap->swapchain.imageCount; ++i) {
		...
		*pInitInfo->pMaterialMap->pApp->pViewport =
		    (VkViewport){0,
		                 0,
		                 (float)pInitInfo->pMaterialMap->swapchain.extent.width,
		                 (float)pInitInfo->pMaterialMap->swapchain.extent.height,
		                 0,
		                 1};
		...
		vkBeginCommandBuffer(pUpdateCommands->pCommandBuffers[i],
		                     &commandBufferBeginInfo);

		/* */
		vkCmdSetViewport(pUpdateCommands->pCommandBuffers[i], 0, 1,
		                 pInitInfo->pMaterialMap->pApp->pViewport);
		vkCmdSetScissor(pUpdateCommands->pCommandBuffers[i], 0, 1,
		                pInitInfo->pMaterialMap->pApp->pScissor);
		vkCmdBeginRenderPass(pUpdateCommands->pCommandBuffers[i],
		...
		SgResource* pResource =
		    hashmap_get(pInitInfo->pResourceMap->pResourceMap, &findResource);
		if (pResource == NULL) {
			sgLogError("Index Resource Not Present");
		}
		vkCmdBindIndexBuffer(pUpdateCommands->pCommandBuffers[i],
		...
		hashmap_scan(pInitInfo->pMaterialMap->pMaterialRenderObjectMap,
		             renderPassBindRenderObjects, &renderObjectInfo);
...
SgBool sgAppUpdate(SgAppUpdateInfo* pUpdateInfo) {
	...
	glfwPollEvents();
	/* Retrieve Image */
	vkWaitForFences(pApp->device, 1, &pApp->pFrameFences[pApp->currentFrame],
	                VK_TRUE, UINT64_MAX);

	VkResult res =
	    vkAcquireNextImageKHR(pApp->device, pSwapchain->swapchain, UINT64_MAX,
	                          pApp->pFrameReadySemaphore[pApp->currentFrame],
	                          VK_NULL_HANDLE, &pApp->frameImageIndex);

	/* Draw to Frame Image */
	if (pApp->pFrameImageInFlightFences[pApp->frameImageIndex]
	    != VK_NULL_HANDLE) {
		vkWaitForFences(pApp->device, 1,
		                &pApp->pFrameImageInFlightFences[pApp->currentFrame],
		                VK_TRUE, UINT64_MAX);
	}
	pApp->pFrameImageInFlightFences[pApp->frameImageIndex] =
	    pApp->pFrameFences[pApp->currentFrame];
	...
	if (res == VK_ERROR_OUT_OF_DATE_KHR || res == VK_SUBOPTIMAL_KHR) {
		...
		sgCreateSwapchain(pApp, &swapchainCreateInfo,
...
void sgDestroyResource(const SgApp* pApp, SgResource** ppResource) {
	...
	if (pResource->type & SG_RESOURCE_TYPE_IS_IMAGE_MASK) {
		...
		if (pResource->stagingBuffer.allocation) {
			...
			vmaDestroyBuffer(pApp->allocator, pResource->stagingBuffer.buffer,
			                 pResource->stagingBuffer.allocation);
		} else {
			vmaUnmapMemory(pApp->allocator, pResource->image.allocation);
		}
		vmaDestroyImage(pApp->allocator, pResource->image.image,
		                pResource->image.allocation);
	} else {
		if (pResource->type & SG_RESOURCE_TYPE_REQIRE_STAGING_MASK) {
			vmaUnmapMemory(pApp->allocator, pResource->stagingBuffer.allocation);
			vmaDestroyBuffer(pApp->allocator, pResource->stagingBuffer.buffer,
			                 pResource->stagingBuffer.allocation);
			vmaDestroyBuffer(pApp->allocator, pResource->dataBuffer.buffer,
			                 pResource->dataBuffer.allocation);
		} else {
...
#ifdef _DEBUG
	vkDestroyDebugUtilsMessengerEXT(pApp->instance, pApp->debugCallback,
	                                VK_NULL_HANDLE);
#endif
